...
double eval_1d_exact_p(double x,double p_0,double p_x,double a_px,double L)
{
...
double eval_1d_exact_u(double x,double u_0,double u_x,double a_ux,double L)
{
  const double pi = acos(-1);
...
double eval_1d_exact_rho(double x,double rho_0,double rho_x,double a_rhox,double L)
{ 
  const double pi = acos(-1);
  ...
  double L)
{
  ...
  RHO = rho_0 + rho_x * sin(a_rhox * pi * x / L) + rho_y * cosl(a_rhoy * pi * y / L) + rho_z * sin(a_rhoz * pi * z / L);
  U = u_0 + u_x * sin(a_ux * pi * x / L) + u_y * cosl(a_uy * pi * y / L) + u_z * cosl(a_uz * pi * z / L);
  V = v_0 + v_x * cosl(a_vx * pi * x / L) + v_y * sin(a_vy * pi * y / L) + v_z * sin(a_vz * pi * z / L);
  W = w_0 + w_x * sin(a_wx * pi * x / L) + w_y * sin(a_wy * pi * y / L) + w_z * cosl(a_wz * pi * z / L);
  P = p_0 + p_x * cosl(a_px * pi * x / L) + p_y * sin(a_py * pi * y / L) + p_z * cosl(a_pz * pi * z / L);

  Q_e = -a_px * pi * p_x * Gamma * U * sin(a_px * pi * x / L) / (Gamma - 0.1e1) / L + a_py * pi * p_y * Gamma * V * cosl(a_py * pi * y / L) / (Gamma - 0.1e1) / L - a_pz * pi * p_z * Gamma * W * sin(a_pz * pi * z / L) / (Gamma - 0.1e1) / L + (U * U + V * V + W * W) * a_rhox * pi * rho_x * U * cosl(a_rhox * pi * x / L) / L / 0.2e1 - (U * U + V * V + W * W) * a_rhoy * pi * rho_y * V * sin(a_rhoy * pi * y / L) / L / 0.2e1 + (U * U + V * V + W * W) * a_rhoz * pi * rho_z * W * cosl(a_rhoz * pi * z / L) / L / 0.2e1 - (-0.3e1 * a_ux * u_x * cosl(a_ux * pi * x / L) - a_vy * v_y * cosl(a_vy * pi * y / L) + a_wz * w_z * sin(a_wz * pi * z / L)) * pi * RHO * U * U / L / 0.2e1 - (a_uy * u_y * sin(a_uy * pi * y / L) + a_vx * v_x * sin(a_vx * pi * x / L)) * pi * RHO * U * V / L - (a_uz * u_z * sin(a_uz * pi * z / L) - a_wx * w_x * cosl(a_wx * pi * x / L)) * pi * RHO * U * W / L - (-a_ux * u_x * cosl(a_ux * pi * x / L) - 0.3e1 * a_vy * v_y * cosl(a_vy * pi * y / L) + a_wz * w_z * sin(a_wz * pi * z / L)) * pi * RHO * V * V / L / 0.2e1 + (a_vz * v_z * cosl(a_vz * pi * z / L) + a_wy * w_y * cosl(a_wy * pi * y / L)) * pi * RHO * V * W / L - (-a_ux * u_x * cosl(a_ux * pi * x / L) - a_vy * v_y * cosl(a_vy * pi * y / L) + 0.3e1 * a_wz * w_z * sin(a_wz * pi * z / L)) * pi * RHO * W * W / L / 0.2e1 - (-a_ux * u_x * cosl(a_ux * pi * x / L) - a_vy * v_y * cosl(a_vy * pi * y / L) + a_wz * w_z * sin(a_wz * pi * z / L)) * pi * Gamma * P / (Gamma - 0.1e1) / L;
  ...
  double L)
{
  ...
  Q_u = a_rhox * pi * rho_x * U * U * cosl(a_rhox * pi * x / L) / L - a_rhoy * pi * rho_y * U * V * sin(a_rhoy * pi * y / L) / L + a_rhoz * pi * rho_z * U * W * cosl(a_rhoz * pi * z / L) / L - a_uy * pi * u_y * RHO * V * sin(a_uy * pi * y / L) / L - a_uz * pi * u_z * RHO * W * sin(a_uz * pi * z / L) / L - a_px * pi * p_x * sin(a_px * pi * x / L) / L + (0.2e1 * a_ux * u_x * cosl(a_ux * pi * x / L) + a_vy * v_y * cosl(a_vy * pi * y / L) - a_wz * w_z * sin(a_wz * pi * z / L)) * pi * RHO * U / L;
  ...
  double L)
{
  ...
  Q_v = a_rhox * pi * rho_x * U * V * cosl(a_rhox * pi * x / L) / L - a_rhoy * pi * rho_y * V * V * sin(a_rhoy * pi * y / L) / L + a_rhoz * pi * rho_z * V * W * cosl(a_rhoz * pi * z / L) / L - a_vx * pi * v_x * RHO * U * sin(a_vx * pi * x / L) / L + a_vz * pi * v_z * RHO * W * cosl(a_vz * pi * z / L) / L + a_py * pi * p_y * cosl(a_py * pi * y / L) / L + (a_ux * u_x * cosl(a_ux * pi * x / L) + 0.2e1 * a_vy * v_y * cosl(a_vy * pi * y / L) - a_wz * w_z * sin(a_wz * pi * z / L)) * pi * RHO * V / L;
  return(Q_v);
}

double eval_3d_source_w(
  double x,
  double y,
  double z,
  double u_0,
  ...
  double L)
{
  ...
  Q_w = a_rhox * pi * rho_x * U * W * cosl(a_rhox * pi * x / L) / L - a_rhoy * pi * rho_y * V * W * sin(a_rhoy * pi * y / L) / L + a_rhoz * pi * rho_z * W * W * cosl(a_rhoz * pi * z / L) / L + a_wx * pi * w_x * RHO * U * cosl(a_wx * pi * x / L) / L + a_wy * pi * w_y * RHO * V * cosl(a_wy * pi * y / L) / L - a_pz * pi * p_z * sin(a_pz * pi * z / L) / L + (a_ux * u_x * cosl(a_ux * pi * x / L) + a_vy * v_y * cosl(a_vy * pi * y / L) - 0.2e1 * a_wz * w_z * sin(a_wz * pi * z / L)) * pi * RHO * W / L;
  return(Q_w);
}
  ...
  double L)
{
  ...
  Q_rho = a_rhox * pi * rho_x * U * cosl(a_rhox * pi * x / L) / L - a_rhoy * pi * rho_y * V * sin(a_rhoy * pi * y / L) / L + a_rhoz * pi * rho_z * W * cosl(a_rhoz * pi * z / L) / L + (a_ux * u_x * cosl(a_ux * pi * x / L) + a_vy * v_y * cosl(a_vy * pi * y / L) - a_wz * w_z * sin(a_wz * pi * z / L)) * pi * RHO / L;
