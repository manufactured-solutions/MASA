...
{
  Scalar Q_T = k_0 * cosl(A_x * x) * cosl(B_y * y) * (A_x * A_x + B_y * B_y);
  return Q_T;
}
...
{
  Scalar Q_T = k_0 * cosl(A_x * x) * cosl(B_y * y) * cosl(C_z * z) * (A_x * A_x + B_y * B_y + C_z * C_z);
  return Q_T;
}
...
{
  Scalar Q_T = -pow(cosl(D_t * t), Scalar(2)) * A_x * A_x * k_1 + Scalar(3) * pow(cosl(A_x * x + A_t * t), Scalar(3)) * pow(cosl(D_t * t), Scalar(3)) * A_x * A_x * k_2 - sin(A_x * x + A_t * t) * cosl(D_t * t) * A_t * rho * cp_0 - cosl(A_x * x + A_t * t) * sin(D_t * t) * D_t * rho * cp_0 + (A_x * A_x * k_0 - Scalar(2) * pow(cosl(D_t * t), Scalar(2)) * A_x * A_x * k_2 - sin(A_x * x + A_t * t) * cosl(D_t * t) * A_t * rho * cp_1 - cosl(A_x * x + A_t * t) * sin(D_t * t) * D_t * rho * cp_1) * cosl(A_x * x + A_t * t) * cosl(D_t * t) + (Scalar(2) * A_x * A_x * k_1 - sin(A_x * x + A_t * t) * cosl(D_t * t) * A_t * rho * cp_2 - cosl(A_x * x + A_t * t) * sin(D_t * t) * D_t * rho * cp_2) * pow(cosl(A_x * x + A_t * t), Scalar(2)) * pow(cosl(D_t * t), Scalar(2));
  return Q_T;
}
...
{
  Scalar Q_T = -pow(cosl(B_y * y + B_t * t), Scalar(2)) * pow(cosl(D_t * t), Scalar(2)) * A_x * A_x * k_1 - sin(A_x * x + A_t * t) * cosl(B_y * y + B_t * t) * cosl(D_t * t) * A_t * rho * cp_0 - pow(cosl(A_x * x + A_t * t), Scalar(2)) * pow(cosl(D_t * t), Scalar(2)) * B_y * B_y * k_1 - cosl(A_x * x + A_t * t) * sin(B_y * y + B_t * t) * cosl(D_t * t) * B_t * rho * cp_0 - cosl(A_x * x + A_t * t) * cosl(B_y * y + B_t * t) * sin(D_t * t) * D_t * rho * cp_0 + Scalar(3) * pow(cosl(A_x * x + A_t * t), Scalar(3)) * pow(cosl(B_y * y + B_t * t), Scalar(3)) * pow(cosl(D_t * t), Scalar(3)) * (A_x * A_x + B_y * B_y) * k_2 + (Scalar(2) * A_x * A_x * k_1 - sin(A_x * x + A_t * t) * cosl(B_y * y + B_t * t) * cosl(D_t * t) * A_t * rho * cp_2 + Scalar(2) * B_y * B_y * k_1 - cosl(A_x * x + A_t * t) * sin(B_y * y + B_t * t) * cosl(D_t * t) * B_t * rho * cp_2 - cosl(A_x * x + A_t * t) * cosl(B_y * y + B_t * t) * sin(D_t * t) * D_t * rho * cp_2) * pow(cosl(A_x * x + A_t * t), Scalar(2)) * pow(cosl(B_y * y + B_t * t), Scalar(2)) * pow(cosl(D_t * t), Scalar(2)) + (A_x * A_x * k_0 - Scalar(2) * pow(cosl(B_y * y + B_t * t), Scalar(2)) * pow(cosl(D_t * t), Scalar(2)) * A_x * A_x * k_2 - sin(A_x * x + A_t * t) * cosl(B_y * y + B_t * t) * cosl(D_t * t) * A_t * rho * cp_1 + B_y * B_y * k_0 - Scalar(2) * pow(cosl(A_x * x + A_t * t), Scalar(2)) * pow(cosl(D_t * t), Scalar(2)) * B_y * B_y * k_2 - cosl(A_x * x + A_t * t) * sin(B_y * y + B_t * t) * cosl(D_t * t) * B_t * rho * cp_1 - cosl(A_x * x + A_t * t) * cosl(B_y * y + B_t * t) * sin(D_t * t) * D_t * rho * cp_1) * cosl(A_x * x + A_t * t) * cosl(B_y * y + B_t * t) * cosl(D_t * t);
  return Q_T;
}
...
{
  Scalar Q_T = -sin(A_x * x + A_t * t) * cosl(B_y * y + B_t * t) * cosl(C_z * z + C_t * t) * cosl(D_t * t) * rho * cp_0 * A_t - cosl(A_x * x + A_t * t) * sin(B_y * y + B_t * t) * cosl(C_z * z + C_t * t) * cosl(D_t * t) * rho * cp_0 * B_t - cosl(A_x * x + A_t * t) * cosl(B_y * y + B_t * t) * sin(C_z * z + C_t * t) * cosl(D_t * t) * rho * cp_0 * C_t - cosl(A_x * x + A_t * t) * cosl(B_y * y + B_t * t) * cosl(C_z * z + C_t * t) * sin(D_t * t) * rho * cp_0 * D_t - pow(cosl(B_y * y + B_t * t), Scalar(2)) * pow(cosl(C_z * z + C_t * t), Scalar(2)) * pow(cosl(D_t * t), Scalar(2)) * k_1 * A_x * A_x - pow(cosl(A_x * x + A_t * t), Scalar(2)) * pow(cosl(C_z * z + C_t * t), Scalar(2)) * pow(cosl(D_t * t), Scalar(2)) * k_1 * B_y * B_y - pow(cosl(A_x * x + A_t * t), Scalar(2)) * pow(cosl(B_y * y + B_t * t), Scalar(2)) * pow(cosl(D_t * t), Scalar(2)) * k_1 * C_z * C_z + Scalar(3) * pow(cosl(A_x * x + A_t * t), Scalar(3)) * pow(cosl(B_y * y + B_t * t), Scalar(3)) * pow(cosl(C_z * z + C_t * t), Scalar(3)) * pow(cosl(D_t * t), Scalar(3)) * (A_x * A_x + B_y * B_y + C_z * C_z) * k_2 + (-sin(A_x * x + A_t * t) * cosl(B_y * y + B_t * t) * cosl(C_z * z + C_t * t) * cosl(D_t * t) * rho * cp_1 * A_t - cosl(A_x * x + A_t * t) * sin(B_y * y + B_t * t) * cosl(C_z * z + C_t * t) * cosl(D_t * t) * rho * cp_1 * B_t - cosl(A_x * x + A_t * t) * cosl(B_y * y + B_t * t) * sin(C_z * z + C_t * t) * cosl(D_t * t) * rho * cp_1 * C_t - cosl(A_x * x + A_t * t) * cosl(B_y * y + B_t * t) * cosl(C_z * z + C_t * t) * sin(D_t * t) * rho * cp_1 * D_t + k_0 * A_x * A_x + k_0 * B_y * B_y + k_0 * C_z * C_z - Scalar(2) * pow(cosl(B_y * y + B_t * t), Scalar(2)) * pow(cosl(C_z * z + C_t * t), Scalar(2)) * pow(cosl(D_t * t), Scalar(2)) * k_2 * A_x * A_x - Scalar(2) * pow(cosl(A_x * x + A_t * t), Scalar(2)) * pow(cosl(C_z * z + C_t * t), Scalar(2)) * pow(cosl(D_t * t), Scalar(2)) * k_2 * B_y * B_y - Scalar(2) * pow(cosl(A_x * x + A_t * t), Scalar(2)) * pow(cosl(B_y * y + B_t * t), Scalar(2)) * pow(cosl(D_t * t), Scalar(2)) * k_2 * C_z * C_z) * cosl(A_x * x + A_t * t) * cosl(B_y * y + B_t * t) * cosl(C_z * z + C_t * t) * cosl(D_t * t) + (-sin(A_x * x + A_t * t) * cosl(B_y * y + B_t * t) * cosl(C_z * z + C_t * t) * cosl(D_t * t) * rho * cp_2 * A_t - cosl(A_x * x + A_t * t) * sin(B_y * y + B_t * t) * cosl(C_z * z + C_t * t) * cosl(D_t * t) * rho * cp_2 * B_t - cosl(A_x * x + A_t * t) * cosl(B_y * y + B_t * t) * sin(C_z * z + C_t * t) * cosl(D_t * t) * rho * cp_2 * C_t - cosl(A_x * x + A_t * t) * cosl(B_y * y + B_t * t) * cosl(C_z * z + C_t * t) * sin(D_t * t) * rho * cp_2 * D_t + Scalar(2) * k_1 * A_x * A_x + Scalar(2) * k_1 * B_y * B_y + Scalar(2) * k_1 * C_z * C_z) * pow(cosl(A_x * x + A_t * t), Scalar(2)) * pow(cosl(B_y * y + B_t * t), Scalar(2)) * pow(cosl(C_z * z + C_t * t), Scalar(2)) * pow(cosl(D_t * t), Scalar(2));
  return Q_T;
}
...
{
  Scalar Q_T = Scalar(3) * A_x * A_x * k_2 * pow(cosl(A_x * x), Scalar(3)) + Scalar(2) * A_x * A_x * k_1 * pow(cosl(A_x * x), Scalar(2)) - A_x * A_x * k_1 + (k_0 - Scalar(2) * k_2) * A_x * A_x * cosl(A_x * x);
  return Q_T;
}
...
{
  Scalar Q_T = (Scalar(3) * A_x * A_x + Scalar(3) * B_y * B_y) * k_2 * pow(cosl(A_x * x), Scalar(3)) * pow(cosl(B_y * y), Scalar(3)) + (Scalar(2) * A_x * A_x + Scalar(2) * B_y * B_y) * k_1 * pow(cosl(A_x * x), Scalar(2)) * pow(cosl(B_y * y), Scalar(2)) - (pow(cosl(B_y * y), Scalar(2)) * A_x * A_x + pow(cosl(A_x * x), Scalar(2)) * B_y * B_y) * k_1 + (k_0 * A_x * A_x + k_0 * B_y * B_y - Scalar(2) * pow(cosl(B_y * y), Scalar(2)) * k_2 * A_x * A_x - Scalar(2) * pow(cosl(A_x * x), Scalar(2)) * k_2 * B_y * B_y) * cosl(A_x * x) * cosl(B_y * y);
  return Q_T;
}
...
{
  Scalar Q_T = (Scalar(3) * A_x * A_x + Scalar(3) * B_y * B_y + Scalar(3) * C_z * C_z) * k_2 * pow(cosl(A_x * x), Scalar(3)) * pow(cosl(B_y * y), Scalar(3)) * pow(cosl(C_z * z), Scalar(3)) + (Scalar(2) * A_x * A_x + Scalar(2) * B_y * B_y + Scalar(2) * C_z * C_z) * k_1 * pow(cosl(A_x * x), Scalar(2)) * pow(cosl(B_y * y), Scalar(2)) * pow(cosl(C_z * z), Scalar(2)) - (pow(cosl(B_y * y), Scalar(2)) * pow(cosl(C_z * z), Scalar(2)) * A_x * A_x + pow(cosl(A_x * x), Scalar(2)) * pow(cosl(C_z * z), Scalar(2)) * B_y * B_y + pow(cosl(A_x * x), Scalar(2)) * pow(cosl(B_y * y), Scalar(2)) * C_z * C_z) * k_1 + (k_0 * A_x * A_x + k_0 * B_y * B_y + k_0 * C_z * C_z - Scalar(2) * pow(cosl(B_y * y), Scalar(2)) * pow(cosl(C_z * z), Scalar(2)) * k_2 * A_x * A_x - Scalar(2) * pow(cosl(A_x * x), Scalar(2)) * pow(cosl(C_z * z), Scalar(2)) * k_2 * B_y * B_y - Scalar(2) * pow(cosl(A_x * x), Scalar(2)) * pow(cosl(B_y * y), Scalar(2)) * k_2 * C_z * C_z) * cosl(A_x * x) * cosl(B_y * y) * cosl(C_z * z);
  return Q_T;
}
