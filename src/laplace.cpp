...
{
  Scalar f;
  f  = 2*std::pow(Lx-x,2) - 8*(Lx-x)*(Lx+x) + 2*std::pow(Lx+x,2);
  f += 2*std::pow(Ly-y,2) - 8*(Ly-y)*(Ly+y) + 2*std::pow(Ly+y,2);
  return f;
}
...
{
  Scalar phi;
  phi = std::pow(Ly-y,2)*std::pow(Ly+y,2) + std::pow(Lx-x,2)*std::pow(Lx+x,2);
  return phi;
}
