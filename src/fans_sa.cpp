...
Scalar MASA::fans_sa_steady_wall_bounded<Scalar>::eval_q_nu(Scalar x,Scalar y)
{
  ...
  chi = RHO * NU_SA / mu;
  f_v1 = std::pow(chi, Scalar(0.3e1)) / (std::pow(chi, Scalar(0.3e1)) + std::pow(c_v1, Scalar(0.3e1)));
  f_v2 = Scalar(0.1e1) - chi / (Scalar(0.1e1) + chi * f_v1);
  Omega = std::sqrt(std::pow(Scalar(0.196e3) * x * x * y_plus * d_eqplus_yplus * std::cos(A / u_inf * u_eq) + Scalar(0.15e2) * eta_v * y * y, Scalar(0.2e1)) * u_tau * u_tau * std::pow(x, Scalar(-0.4e1)) * std::pow(y, Scalar(-0.2e1))) / 0.196e3;
  Sm_orig = NU_SA * std::pow(kappa, Scalar(-0.2e1)) * std::pow(d, Scalar(-0.2e1)) * f_v2;
  Sm1 = Sm_orig;
  // nick: making change to equality after consulting with kemelli
  Sm2 = Omega * (c_v2 * c_v2 * Omega + c_v3 * Sm_orig) / ((c_v3 + (Scalar(-0.1e1)) * Scalar(0.20e1) * c_v2) * Omega - Sm_orig);
  if (-c_v2 * Omega <= Sm_orig)
    Sm = Sm1;
  else
    Sm = Sm2;
  S_sa = Sm + Omega;
  r = NU_SA / S_sa * std::pow(kappa, Scalar(-0.2e1)) * std::pow(d, Scalar(-0.2e1));
  g = r + c_w2 * (std::pow(r, Scalar(0.6e1)) - r);
  f_w = g * std::pow((Scalar(0.1e1) + std::pow(c_w3, Scalar(0.6e1))) / (std::pow(g, Scalar(0.6e1)) + std::pow(c_w3, Scalar(0.6e1))), Scalar(0.1e1) / Scalar(0.6e1));
  Q_nu = -r_T * (Scalar) (Gamma - Scalar(1)) * M_inf * M_inf * T_inf * y_plus * u_tau * d_eqplus_yplus * NU_SA * RHO * U * V * std::cos(A / u_inf * u_eq) * std::pow(u_inf, Scalar(-0.2e1)) / y / T + (Scalar) (Gamma - Scalar(1)) * (y_plus * d_eqplus_yplus + u_eq_plus) * y * kappa * r_T * M_inf * M_inf * T_inf * u_tau * u_tau * NU_SA * RHO * U * std::cos(A / u_inf * u_eq) / sigma * std::pow(u_inf, Scalar(-0.2e1)) * std::pow(x, Scalar(-0.2e1)) / T / Scalar(0.196e3) - (Scalar) (Gamma - Scalar(1)) * (Scalar(0.2e1) * alpha * y - kappa * u_tau) * r_T * M_inf * M_inf * T_inf * y_plus * u_tau * d_eqplus_yplus * NU_SA * RHO * U * std::cos(A / u_inf * u_eq) / sigma * std::pow(u_inf, Scalar(-0.2e1)) / y / T + (Scalar) (Gamma - Scalar(1)) * (y_plus * d_eqplus_yplus + u_eq_plus) * r_T * M_inf * M_inf * T_inf * u_tau * NU_SA * RHO * U * U * std::cos(A / u_inf * u_eq) * std::pow(u_inf, Scalar(-0.2e1)) / x / T / Scalar(0.14e2) - (Scalar) (Scalar(1) + c_b2) * y * y * kappa * kappa * u_tau * u_tau * RHO / sigma * std::pow(x, Scalar(-0.2e1)) / Scalar(0.196e3) - kappa * u_tau * y * RHO * U / x / Scalar(0.14e2) - (y_plus * d_eqplus_yplus + u_eq_plus) * u_tau * NU_SA * RHO * std::cos(A / u_inf * u_eq) / x / Scalar(0.14e2) + c_w1 * f_w * NU_SA * NU_SA * RHO * std::pow(d, Scalar(-0.2e1)) + RHO * NU_SA * V / y - S_sa * c_b1 * NU_SA * RHO + (Scalar(-0.2e1) * alpha * y + kappa * u_tau) * RHO * V - std::pow(Scalar(0.2e1) * alpha * y - kappa * u_tau, Scalar(0.2e1)) * (Scalar) (Scalar(1) + c_b2) * RHO / sigma + (RHO * NU_SA + mu) * (Scalar(0.392e3) * alpha * x * x - Scalar(0.15e2) * kappa * u_tau * y) / sigma * std::pow(x, Scalar(-0.2e1)) / 0.196e3;
  return(Q_nu);
}
